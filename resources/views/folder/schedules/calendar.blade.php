...
$(document).ready(function() {
    ...
    function updateCellContent($cell, data) {
        const content = data.schedule ? 
            `<div class="schedule-badge ${data.is_weekend ? 'bg-secondary' : 'bg-primary'} text-white p-1 rounded">
                <div class="small">${data.schedule.work_hour_type}</div>
                <div class="small">${data.schedule.start_time} - ${data.schedule.end_time}</div>
                ${data.schedule.notes ? '<div class="small"><i class="bi bi-chat-text"></i></div>' : ''}
            </div>` :
            `<div class="text-muted small">${data.is_weekend ? 'Weekend' : 'Non planifié'}</div>`;
