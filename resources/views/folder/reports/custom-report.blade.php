...
<!-- Suppression des liens DataTables -->
...
$(document).ready(function() {
    ...
    function updateReportSummary(data) {
        ...
        data.forEach(function(employee) {
            avgPresenceRate += employee.presence_data.rate || 0;
            avgPonctualiteRate += employee.ponctualite_data.rate || 0;
            totalWorkingDays += employee.total_days || 0;
        });
        
        avgPresenceRate = totalEmployees > 0 ? Math.round(avgPresenceRate / totalEmployees) : 0;
        avgPonctualiteRate = totalEmployees > 0 ? Math.round(avgPonctualiteRate / totalEmployees) : 0;
        
        $('#total-employees').text(totalEmployees);
        $('#avg-presence-rate').text(avgPresenceRate + '%');
        $('#avg-ponctualite-rate').text(avgPonctualiteRate + '%');
        $('#total-days').text(totalWorkingDays);
        ...
        $('#period-info').text('Période: ' + startDate + ' au ' + endDate + ' (' + daysDiff + ' jours)');
    ...
    function displayReportData(data) {
        ...
        data.forEach(function(employee) {
            ...
            var row = 
                '<tr>' +
                '<td class="text-center fw-bold">' + employee.order_number + '</td>' +
                '<td>' + 
                ...
                '<td class="text-center">' +
                    ...
                    '</span>' +
                '</td>' +
                '<td class="small text-start">' + (employee.observation || 'Aucune observation') + '</td>' +
                '</tr>';
        ...
        var avgPresenceRate = data.length > 0 ? Math.round(totalPresenceRate / data.length) : 0;
        var avgPonctualiteRate = data.length > 0 ? Math.round(totalPonctualiteRate / data.length) : 0;
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showSweetAlert('success', 'Succès', 
                        'Rapport généré avec succès. ' + response.total_employees + ' employé(s) analysé(s).');
                } else {
    ...
    function exportToPdf() {
        ...
        showSweetAlert('question', 'Exporter en PDF', message, true).then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('reports.custom.export.pdf') }}",
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                        start_date: startDate,
