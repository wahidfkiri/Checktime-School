            ...
            <section class="section">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <!-- Tableau des retards simplifié -->
                        <div class="table-responsive">
                            <table id="retards-table" class="table table-striped table-hover" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Date</th>
                                        <th>Jour</th>
...
$(document).ready(function() {
    ...
    function simulatePdfProgress(callback) {
        ...
        var steps = [
            ...
            { progress: 30, message: 'Récupération des retards...' },
    ...
    // ========== DATATABLE CONFIGURATION SIMPLIFIÉE ==========
    
    var table = $('#retards-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "{{ route('admin.daily-attendance.retard.data') }}",
            type: 'GET',
            data: function(d) {
            ...
            beforeSend: function() {
                if (!$('#data-progress-container').hasClass('d-none')) {
                    ...
                    showLoading('Chargement des retards...');
                }
            },
            complete: function() {
                hideLoading();
    ...
    $('#apply_filters').on('click', function() {
        ...
        showDataProgress('Chargement des retards...', details);
        
        // Simulation des étapes de progression
        var steps = [
            { progress: 10, message: 'Initialisation de la requête...' },
            { progress: 25, message: 'Récupération des retards...' },
